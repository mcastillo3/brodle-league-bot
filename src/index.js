...
client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    ...
    if (result === "new") {
      ...
      const label =
        LABELS[cid] || message.member?.displayName || message.author.username;
      ...
      const isMilestone =
        milestones.has(games) || (games >= 1000 && games % 250 === 0);
      if (isMilestone) {
      ...
      if (parsed.score === 1 && !parsed.failed) {
        ...
        if (aces === 1) {
          ...
          await message.channel.send(
            `🎯 An ace from ${label} — their ${aces}th career hole-in-one!`,
          );
        }
      }
    }
...
function makeLabelFor(scores, guild) {
  ...
  return (userId) => {
    ...
    const doc = scores.find((s) => s.userId === userId);
    const name =
      guild?.members.cache.get(userId)?.displayName || doc?.username || "??";
    const words = name.trim().split(/\s+/);
    let init =
      words.length >= 2
        ? (words[0][0] + words[words.length - 1][0]).toUpperCase()
    ...
    footerRows: [
      {
        label: "pts",
        strong: true,
        values: Object.fromEntries(order.map((id) => [id, totals[id] ?? 0])),
      },
      {
        label: "avg",
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    ...
    if (interaction.commandName === "week") {
      ...
      const file = new AttachmentBuilder(built.png, {
        name: `week-${built.weekId}.png`,
      });
      ...
      if (id1 === id2) {
        await interaction.editReply(
          "That would be shadow-boxing. Pick two different players.",
        );
        return;
      }
      const [s1, s2] = await Promise.all([
        db.scoresForPlayer(id1),
        db.scoresForPlayer(id2),
      ]);
      const labelFor = makeLabelFor([...s1, ...s2], interaction.guild);
      const [n1, n2] = [labelFor(id1), labelFor(id2)];
      if (!s1.length || !s2.length) {
        await interaction.editReply(
          `Not enough data — ${!s1.length ? n1 : n2} has no recorded games.`,
        );
        return;
      }
      ...
      if (!v.shared) {
        await interaction.editReply(
          `${n1} and ${n2} have never played the same puzzle. Ships in the night.`,
        );
        return;
      }
      const dailyEdge =
        v.aWins === v.bWins ? "DEAD EVEN" : v.aWins > v.bWins ? n1 : n2;
      const lines = [
      ...
      if (found.puzzle >= currentPuzzleNumber(TZ)) {
        await interaction.editReply(
          "🤐 Nice try. Rule #3: NO SPOILING. Ask me again tomorrow.",
        );
        return;
      }
      ...
      if (!scores.length) {
        ...
        body +=
          ...
          sorted
            .map(
              (s) =>
                `${s.failed ? "❌" : s.score <= 2 ? "🌟" : "🟩"} **${labelFor(s.userId)}** — ${s.failed ? "X" : s.score}/6`,
            )
            .join("\n");
        const best = sorted[0],
      ...
      if (scores.length < 20) {
        await interaction.editReply(
          `${name} hasn't played enough to roast. Come back when there's a body of work. 📉`,
        );
        return;
      }
      ...
      if (scores.length < 10) {
        await interaction.editReply(
          "🔮 The mists are cloudy... play more Wordles and your destiny shall reveal itself.",
        );
        return;
      }
      const stats = fun.playerStats(scores);
...
async function postDailyWeek() {
  ...
  await channel.send({
    files: [
      new AttachmentBuilder(built.png, { name: `week-${built.weekId}.png` }),
    ],
  });
...
async function fetchWordForPuzzle(puzzle) {
  ...
  const num = Number.isInteger(data.days_since_launch)
...
async function announceSeasonChampion() {
  ...
  if (!entries.length) {
    console.log(`Season ${endedYear}: no weekly wins recorded, skipping.`);
    return;
  }
  ...
  if (!fresh) {
    console.log(`Season ${endedYear} already announced, skipping.`);
    return;
  }
...
client.once(Events.ClientReady, (c) => {
  ...
  cron.schedule(
    "5 0 1 1 *",
    () => {
      announceSeasonChampion().catch((err) =>
  ...
  archiveWords().catch((err) =>
    console.error("startup word catch-up failed:", err),
  );
  console.log(
    `Scheduled: Mon 4 AM weekly announce, daily 8 PM standings, daily 1 AM word archive, Jan 1 season rollover (${TZ}).`,
  );
});
